...
2.  **Continent(`Continent`)**: This is the categorical variable showing the geographic information of each country. Developed continents, for example Europe and North America, typically have well-established healthcare infrastructure, which are considered to contribute to higher life expectancy. Factors like clean water access, education resources, nutrition and lifestyle factors can significantly impact life expectancy and vary greatly across continents. @fig-continent reveals that Africa has the highest number of countries represented with the largest portion of the data. Other continents, America and Europe, have fewer countries included, with only 9 countries from Oceania. 

3.  **Percentage Expenditure on Health (`PercentageExpenditure`)**: This variable reflects the expenditure allocated to health-related expenditures as a percentage of GDP per capita.

4.  **Total expenditure (`TotalExpenditure`)**: This variable shows the general government expenditure on health as a percentage of total government expenditure. This is considered as a reflection of economic situation of a country. A higher percentage indicates that a country places significant emphasis on healthcare as a key area of public investment.

5. **Income composition of resource (`IncomeComposition`)**: his is a numerical variable representing the income component of the Human Development Index (HDI).

6. **Schooling (`Schooling`)**: This is a numerical variable showing the number of education per person.  
...
numbercountry <- ggplot(country_counts, aes(x = Status, y = n, fill = Status)) +
  ...
  geom_text(aes(label = n), vjust = -0.5,size = 2.5) +  # Add count labels on top of bars
  labs(
...
#| fig-cap: "Distribution of expenditure allocated to health-related expenditures as a percentage of GDP per capita. Africa has the highest number of countries represented, followed by Asia among all 5 continents. The distribution of count of data aslo shows that a significant portion of the data is focused on African and Asian nations.  "
#| echo: false
#| eval: true
#| warning: false
#| message: false
