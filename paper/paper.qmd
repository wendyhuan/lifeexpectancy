...
@fig-cleaned shows the average of life expectancy under male, female, 6 different regions and 4 income groups. 
...
# Create 10-year bins for Life Expectancy
lerange <- lelecleaned_data %>%
  mutate(LifeExpectancyRange = cut(`Life Expectancy`, 
                                   breaks = seq(floor(min(`Life Expectancy`, na.rm = TRUE)), 
                                   ceiling(max(`Life Expectancy`, na.rm = TRUE)), by = 10), 
                                   include.lowest = TRUE))

# Calculate percentages for each bin
life_expectancy_distribution <- lerange %>%
  count(LifeExpectancyRange) %>%
  mutate(Percentage = 100 * n / sum(n))

# Plot the histogram with percentages
ggplot(life_expectancy_distribution, aes(x = LifeExpectancyRange, y = Percentage, fill = LifeExpectancyRange)) +
  geom_bar(stat = "identity", color = "white", alpha = 0.8) +
  labs(
    title = "Distribution of Life Expectancy by Age Range",
    x = "Life Expectancy (10-Year Ranges)",
    y = "Percentage"
  ) +
  theme_minimal() +
